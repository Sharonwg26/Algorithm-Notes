...
* Rank  給定一個由二維平面點所構成的集合S，點A之秩\(rank\)定義為集合S中有多少個點被A所支配。  ![](https://i.imgur.com/wUcqYL3.png)
...
* 排序演算法  T\(n\)=2T\(n/2\)+c1nlogn+c2n=2T\(n/2\)+cn logn  =2\(2T\(n/4\)+c\(n/2\)log\(n/2\)\)+cnlogn=4T\(n/4\)+cnlog\(n/2\)+cnlogn  = nT\(1\) + cn\(log n + log \(n/2\)+ log \(n/4\) +…+ log 2\)  = nT\(1\) + cn \(log n \(log n+ log 2\)\)/2 \(其中T\(1\)=1\)  = **O\(nlog2n\)**
...
* 分治演算法  ![](https://i.imgur.com/VJKVHVg.png)
